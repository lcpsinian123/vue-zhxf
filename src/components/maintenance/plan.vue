<template>
	<div class='containers cententpadding'>
		...
		<div class="rightHead">
			<div style='background-color:#fff;'>
				<div class='searchMeun'>
					<div>
						<Input search style='width:500px;margin-left:10px' enter-button placeholder="搜索巡检计划名称..." v-model="ptpname" @on-search="getplanlist" />
					</div>
				</div>
					...
					<div v-if='formValidate.category=="2"'>
						<FormItem label="计划执行时间:">
							<Row>
								<Col span="11">
								<FormItem prop="date">
									<TimePicker style='width:100%' format="HH:mm" type="time" placeholder="计划开始时间..." v-model="formValidate.ipmstartTime"></TimePicker>
								</FormItem>
								</Col>
								<Col span="2" style="text-align: center">-</Col>
								<Col span="11">
								<FormItem prop="time">
									<TimePicker style='width:100%' format="HH:mm" type="time" placeholder="计划结束时..." v-model="formValidate.ipmendTime"></TimePicker>
								</FormItem>
								</Col>
							</Row>
						</FormItem>
		...
		<div class='tableBox'>
			<div class='btnclass'>
				<Button class='wdName' @click="addPlan" type="primary" ghost>添加</Button>
					...
					<Modal width="700" id="add" class="plan" @on-ok='updataPlan' v-model="updata">
						...
						<Form ref="formValidate" :model="formValidate" :label-width="120">
							...
							<FormItem label="计划开始日期:" prop="startDay">
								...
								<DatePicker style='width:100%;' type="date" placeholder="计划开始日期..." :value="formValidate.startDay" @on-change="formValidate.startDay=$event"></DatePicker>

							</FormItem>
							<FormItem label="计划结束日期:" prop="endDay">

								<DatePicker style='width:100%;' type="date" placeholder="计划结束日期..." :value="formValidate.endDay" @on-change="formValidate.endDay=$event"></DatePicker>
							...
							<div v-if='formValidate.category=="2"'>
								<FormItem label="计划执行时间:">
									<Row>
										...
										<Col span="11">
										<FormItem prop="time">
											<TimePicker style='width:100%' format="HH:mm" type="time" placeholder="计划结束时间..." v-model="formValidate.ipmendTime"></TimePicker>
										</FormItem>
										</Col>
									</Row>
								</FormItem>
							...
							<div v-if='formValidate.category=="7"'>
								<FormItem label="生成时间:" prop="longTime">
									<div calss='TimeBox' style='display:flex;width:100%;'>
										<div style='width:45%'>
											<Select v-model="formValidate.YearGenerateStart" style="width:1YearGenerateStart00%">
												<Option v-for="item in year" v-model="formValidate.YearGenerateStart" :value="item.value" :key="item.value">{{ item.label }}{{item.value }} </Option>
											</Select>
		...
		methods: {
			...
			submitPlan() {
				...
				if(xcid == '0') {
					...
					console.log(objs);
					objs.generateDayTime = this.formValidate.daytime;
					this.addplan(objs)
				} else if(xcid == '2') {
					objs.startTime = this.formValidate.startDay;
					objs.endTime = this.formValidate.endDay;
					objs.evTimeHour = this.formValidate.allValidHour;
					objs.evTimeDay = this.formValidate.allValidDay;
					objs.ipmendTime = this.formValidate.ipmendTime;
					objs.ipmstartTime = this.formValidate.ipmstartTime;
			...
			addPlan() {
				this.add = true;
				this.formValidate.name = null; // 计划名称
				this.formValidate.isStrict = null; // 严格执行
				this.formValidate.startDay = null; //计划开始时间
				this.formValidate.endDay = null; //计划结束时间
				this.formValidate.userPerson = null; // 巡查ID 
				this.formValidate.allPStatus = null; // 计划状态,
				this.formValidate.dayValidDay = null; // 有效时间天
				this.formValidate.dayValidHour = null; // 有效时间时
				this.formValidate.ipmstartTime = null; //计划执行开始时间
				this.formValidate.ipmendTime = null; //计划结束时间
				this.formValidate.generateDayTime = null; //生成时间
				this.formValidate.YearGenerateStart = null; //时间间隔
			...
			updataPlan(obj) {
				...
				var objs = {
					...
					evTimeDay: this.formValidate.allValidDay || null, // 有效时间天
					evTimeHour: this.formValidate.allValidHour || null, // 有效时间时
					guid: this.formValidate.guid
				}
				if(xcid == '0') {
					this.updateplan(objs);
				} else if(xcid == '1') {
					console.log(objs);
					objs.generateDayTime = this.formValidate.daytime;
					this.updateplan(objs)
...
//				this.detail = true
				...
				this.formValidate.guid = data.guid; //id
				if(this.formValidate.category == '2') {
					this.formValidate.data = data.startTime;
					this.formValidate.time = data.endTime;
					this.formValidate.ipmendTime = data.ipmendTime;
					this.formValidate.ipmstartTime = data.ipmstartTime;
					this.formValidate.longTime = data.generateDayTime;
					this.addplan(objs)
				} else if(this.formValidate.category == '3') {
					this.formValidate.daytime = data.generateDayTime;
					...
					this.formValidate.weekdate = data.generateTime.toString(); // 时间间隔
